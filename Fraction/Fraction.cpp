...
void Fraction::set_denominator(int denominator)				//фильтрация данных, так как на ноль делить нельзя
{
	if (denominator == 0)denominator = 1;
	this->denominator = denominator;
}
//							Constructors
Fraction::Fraction(int integer, int numerator, int denominator) //вызывается во всех трех ситуациях, потому что за счет значений по умолчанию, но если убрать - все работает
{
	this->integer = integer;
	this->numerator = numerator;
...
Fraction& Fraction::to_proper(int integer, int numerator, int denominator)//целую часть интегрирует в числитель
{
	int res = 0;
	if (this->integer != 0)
	{
